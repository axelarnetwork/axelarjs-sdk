...
function App() {
  ...
  const getDepositAddress = useCallback(async (destinationAddress?: string) => {
...
const getParameters = (destinationAddress: string, sourceChainName: string = "terra", destinationChainName: string = "avalanche", asset_common_key: string = "uusd") => {
	...
	* */
  const chainInfoList: ChainInfo[] = ChainList.map((chain: Chain) => chain.chainInfo);
  const terraChain: ChainInfo = chainInfoList.find((chainInfo: ChainInfo) => chainInfo.chainName.toLowerCase() === sourceChainName.toLowerCase()) as ChainInfo;
  const avalancheChain: ChainInfo = chainInfoList.find((chainInfo: ChainInfo) => chainInfo.chainName.toLowerCase() === destinationChainName.toLowerCase()) as ChainInfo;
